...
trait UploadableTrait
{
    ...
    public function uploadFile(UploadedFile $file, $path = 'other', $uploadDisk = 'local')
    {
    ...
    public function destroyFile($destinationTarget, $uploadDisk = 'local')
    {
        $storage = \Storage::disk($uploadDisk);
    ...
    public function getTarget($path)
    {
        ...
        if (!property_exists($this, 'ableFolder') || !$this->ableFolder) {
            $this->ableFolder = 'images';
        }
        $path = $this->ableFolder . '/' . $path;
